...
inline crash_handler_internal(int *do_exit)
{
	...
	if (TSD_GET(jmp_set,0)) {
		...
		PRINT_ERROR_SAFE("Warning: signal appeared to be generated by internal latrace routine (%s).\n",
			TSD_GET(fault_reason,NULL));

		if (TSD_GET(last_operation,-1) >= 0)
			PRINT_ERROR_SAFE("Last known operation before crash: %s / %s\n", TSD_GET(last_symbol,NULL),
				(!TSD_GET(last_operation,-1) ? "entry" : "exit"));

		PRINT_ERROR_SAFE("%s", "Exiting immediately.\n");
		...
		PRINT_ERROR_SAFE("%s", "Warning: signal appeared to be delivered outside of user custom code.\n");
...
STATIC void triangulate_pc(unsigned long pc)
{
	...
	PRINT_ERROR_SAFE("SANDWICH: %p [%s] <-> %p [%s] <-> %p [%s]\n",
...
{
	PRINT_ERROR_SAFE("Warning: caught potentially fatal signal: %d / %ld\n", signo, syscall(SYS_gettid));
...
#define PREG(r)	(unsigned long)mcontext.gregs[r]
	...
	PRINT_ERROR_SAFE("Warning: caught potentially fatal signal: %d (code = %d (%s), addr = %p) / thread %ld; pc=0x%lx\n",
		signo, si->si_code, more_info, si->si_addr, syscall(SYS_gettid), pc);
	PRINT_ERROR_SAFE("Warning: register dump[ rax = 0x%lx, rbx = 0x%lx, rcx = 0x%lx, rdx = 0x%lx ]\n",
		PREG(REG_RAX), PREG(REG_RBX), PREG(REG_RCX), PREG(REG_RDX));
	PRINT_ERROR_SAFE("Warning: register dump[ rdi = 0x%lx, rsi = 0x%lx, rbp = 0x%lx, rsp = 0x%lx ]\n",
		PREG(REG_RDI), PREG(REG_RSI), PREG(REG_RBP), PREG(REG_RSP));
	PRINT_ERROR_SAFE("Warning: register dump[ r8 = 0x%lx, r9 = 0x%lx, r10 = 0x%lx, r11 = 0x%lx ]\n",
		PREG(REG_R8), PREG(REG_R9), PREG(REG_R10), PREG(REG_R11));
	PRINT_ERROR_SAFE("Warning: register dump[ r12 = 0x%lx, r13 = 0x%lx, r14 = 0x%lx, r15 = 0x%lx ]\n",
...
#ifdef USE_LIBUNWIND
		PRINT_ERROR_SAFE("%s", "Backtrace provided by libunwind:\n");
		backtrace_unwind(ucontext);
#endif

	if (gdb_once)
		PRINT_ERROR_SAFE("%s", "GDB already seems to be launched... skipping prompt.\n");
/*	else {
		size_t sleep_val = 2;
		char tid_buf[16];

		gdb_once = 1;
		PRINT_ERROR_SAFE("Type anything to spawn GDB (%d / %ld).\n", getpid(), syscall(SYS_gettid));
		getchar();
		snprintf(tid_buf, sizeof(tid_buf), "%ld", syscall(SYS_gettid));
		PRINT_ERROR_SAFE("Launching GDB and sleeping for %zu seconds...\n", sleep_val);
